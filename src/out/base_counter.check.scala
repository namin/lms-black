...
class staged$0 extends Fun[NoRep] with (Value => Value) {
  ...
  def fun[R[_]:Ops](m: MEnv, x0:Value)(implicit ev: Convert[NoRep,R]): R[Value] = {
    ...
    val x22 = o.makeFun(m, new Fun[R] { def fun[R1[_]:Ops](implicit ev_1: Convert[R,R1]) = { (m: MEnv) => {(x4: R[Value]) =>
          ...
          val x8 = o1.app(MEnv(11), o1.lift(Prim("+")), x6, P(N, P(P(P(S("c"), Code(x2)), N), P(Cell(13), N))), mkCont[R1]{(x7: R1[Value]) =>
            x7: R1[Value]
          })
          val x9 = o1.cellSet(x2, x8)
          val x10 = o1.cellRead(x2)
          val x11 = o1.makePair(x10, o1.lift(P(I(1), N)))
          val x13 = o1.app(MEnv(24), o1.lift(Prim("+")), x11, P(N, P(P(P(S("c"), Code(x2)), N), P(Cell(13), N))), mkCont[R1]{(x12: R1[Value]) =>
            x12: R1[Value]
          })
          val x14 = o1.cellSet(x2, x13)
          val x15 = o1.cellRead(x2)
          val x16 = o1.makePair(x15, o1.lift(P(I(1), N)))
          val x18 = o1.app(MEnv(37), o1.lift(Prim("+")), x16, P(N, P(P(P(S("c"), Code(x2)), N), P(Cell(13), N))), mkCont[R1]{(x17: R1[Value]) =>
            x17: R1[Value]
          })
