...
class staged$0 extends Fun[NoRep] with (Value => Value) {
  ...
  def fun[R[_]:Ops](m: MEnv, x0:Value)(implicit ev: Convert[NoRep,R]): R[Value] = {
    ...
    val x1 = o.getCar(x0)
    val x2 = x1
    val x14 = o.makeFun(m, new Fun[R] { def fun[R1[_]:Ops](implicit ev_1: Convert[R,R1]) = { (m: MEnv) => {(x4: R[Value]) =>
    ...
    val x39 = o.makeFun(m, new Fun[R] { def fun[R1[_]:Ops](implicit ev_1: Convert[R,R1]) = { (m: MEnv) => {(x15: R[Value]) =>
          val o1 = implicitly[Ops[R1]]
          implicit def convert_ev_1(x: R[Value]): R1[Value] = ev_1.convert(x)
          val x16 = o1.getCar(x15)
          val x17 = x16
          val x19 = x1
          val x34 = o1.makeFun(m, new Fun[R1] { def fun[R2[_]:Ops](implicit ev1_2: Convert[R1,R2]) = { (m: MEnv) => {(x20: R1[Value]) =>
